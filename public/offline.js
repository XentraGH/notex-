/**
 * NoteX Offline Support for Nativefier
 * 
 * This script provides full offline functionality:
 * - Caches notes and user data to localStorage when online
 * - Loads cached data when offline
 * - Shows offline indicator when working offline
 * - Shows popup for online-only features (sharing)
 * - Syncs offline changes when back online
 * 
 * This file mirrors the inline script in layout.tsx
 */
...
(function() {
  ...
  window.NoteXOffline = {
    getNotes: mergeNotes,
    getUser: function() { return loadFromStorage(STORAGE_KEYS.USER); },
    clearAll: function() {
      removeFromStorage(STORAGE_KEYS.NOTES);
      removeFromStorage(STORAGE_KEYS.ONLINE_NOTES);
      removeFromStorage(STORAGE_KEYS.USER);
      removeFromStorage(STORAGE_KEYS.SYNC_QUEUE);
    },
    showPopup: showOfflinePopup,
    sync: syncOfflineChanges
  };
